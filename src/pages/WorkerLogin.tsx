...
const WorkerLogin = () => {
  ...
  const { signInWorker, user, loading } = useAuth();
  const [phone, setPhone] = useState('');
  const [password, setPassword] = useState('');
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    if (!loading && user) navigate('/worker/dashboard');
  }, [user, loading, navigate]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitting(true);
    const { error } = await signInWorker(phone, password);
    setSubmitting(false);
    if (error) {
      toast.error(error);
    } else {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-12 md:py-20 max-w-sm mx-auto">
        ...
        >
          <h1 className="font-heading text-2xl font-bold text-foreground text-center">{t('worker_login')}</h1>
          <p className="text-center text-muted-foreground text-sm mt-2">
            {t('dont_have_account')} <Link to="/worker/signup" className="text-primary font-medium hover:underline">{t('sign_up')}</Link>
          </p>
